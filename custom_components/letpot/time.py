...
TIME_SENSORS: tuple[LetPotTimeEntityDescription, ...] = (
    LetPotTimeEntityDescription(
        ...
        name="Light off",
        value_fn=lambda status: status.light_schedule_end if hasattr(status, "light_schedule_end") else None,
        set_value_fn=(
        ...
        supported_fn=lambda coordinator: hasattr(coordinator.data, "light_schedule_end"),
    ),
    LetPotTimeEntityDescription(
        key="light_schedule_start",
        name="Light on",
        value_fn=lambda status: status.light_schedule_start if hasattr(status, "light_schedule_start") else None,
        set_value_fn=(
            lambda device_client, serial, value: device_client.set_light_schedule(
        ...
        entity_category=EntityCategory.CONFIG,
        supported_fn=lambda coordinator: hasattr(coordinator.data, "light_schedule_start"),
    ),
)
