...
from letpot.models import CycleWateringMode, DeviceFeature, LightMode, LetPotWateringSystemStatus, TemperatureUnit
    ...
    LetPotSelectEntityDescription(
        ...
        value_fn=(
            lambda coordinator: (
                ...
                if hasattr(coordinator.data, "temperature_unit")
                and coordinator.data.temperature_unit is not None
                else None
            )
        ),
        set_value_fn=(
    ...
    LetPotSelectEntityDescription(
        ...
        value_fn=(
            lambda coordinator: (
                ...
                if hasattr(coordinator.data, "light_mode")
                and coordinator.data.light_mode is not None
                else None
            )
        ),
        set_value_fn=(
        ...
        entity_category=EntityCategory.CONFIG,
        supported_fn=lambda coordinator: hasattr(coordinator.data, "light_mode"),
    ),
    LetPotSelectEntityDescription(
        key="pump_cycle_mode",
        name="Cycle watering mode",
        options=[x.name.lower() for x in CycleWateringMode],
        value_fn=lambda coordinator: (
            coordinator.data.pump_cycle_mode.name.lower()
            if isinstance(coordinator.data, LetPotWateringSystemStatus)
            else None
        ),
        set_value_fn=(
            lambda device_client, serial, option: device_client.set_pump_cycle_mode(
                serial, CycleWateringMode[option.upper()]
            )
        ),
        entity_category=EntityCategory.CONFIG,
