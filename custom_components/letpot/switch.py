...
SWITCHES: tuple[LetPotSwitchEntityDescription, ...] = (
    LetPotSwitchEntityDescription(
        ...
        supported_fn=lambda coordinator: hasattr(coordinator.data, "system_sound") and coordinator.data.system_sound is not None,
    ),
    LetPotSwitchEntityDescription(
        key="auto_mode",
        name="Auto mode",
    ...
    LetPotSwitchEntityDescription(
        ...
        entity_category=EntityCategory.CONFIG,
        supported_fn=lambda coordinator: hasattr(coordinator.data, "system_on"),
    ),
    LetPotSwitchEntityDescription(
        key="pump_cycling",
        name="Pump",
    ...
    LetPotSwitchEntityDescription(
        ...
        value_fn=lambda status: (
            status.pump_cycle_on
            if isinstance(status, LetPotWateringSystemStatus)
            else None
        ),
        set_value_fn=lambda device_client, serial, value: device_client.set_pump_cycle_on(
            serial, value
        ),
        entity_category=EntityCategory.CONFIG,
